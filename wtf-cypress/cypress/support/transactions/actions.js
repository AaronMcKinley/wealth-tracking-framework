...
const Transactions = {
  ...
  navigateAndAssert: {
    openTransactionsFromDashboard() {
      ...
      cy.contains('tr', /\bSOL\b/).within(() => {
        cy.get('a[href*="transactions"], button').first().click();
      });

      cy.wait('@getTxSOL');
      cy.get('h1,header').should(($el) => {
        expect($el.text()).to.match(/transactions/i);
      });
    },
